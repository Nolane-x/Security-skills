...
def _profile_path(
    *, prefix: str, skill_dir: Path, value: object, field: str, errors: list[str]
    ...
    if Path(text).is_absolute():
        errors.append(f"{prefix}: {label} path escapes skill directory")
        return None, None
...
def _validate_scenario_matrix(*, prefix: str, path: Path, errors: list[str]) -> None:
    if not path.is_file():
    ...
    for index, item in enumerate(scenarios):
        ...
        if _nonempty_string(safe_oracle) and not any(
            term in safe_oracle.lower() for term in SAFE_ORACLE_TERMS
        ):
            errors.append(
                f"{scenario_prefix}: safe_oracle must name a benign synthetic/mock/controlled signal"
            )
        stop_condition = item.get("stop_condition")
        if _nonempty_string(stop_condition) and not any(
            term in stop_condition.lower() for term in STOP_TERMS
        ):
        ...
        if runbook_path is not None:
            if not runbook_path.is_file():
                ...
                try:
                    ...
                    if not any(
                        term in lower for term in ("authorized", "owned", "sandbox", "lab")
                    ):
