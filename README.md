...
> **Stable baseline: Wave 8; Wave 10 depth expansion active** — 83 canonical skills, 20 packs, 12 CI-enforced operator-depth profiles with machine-readable matrices, 36 deterministic benchmark fixtures, and a vendor-neutral cross-agent evaluation harness.
...
83 canonical Agent Skills
      │
      ├──► 12 matrix-enforced operator-depth profiles (Wave 8 + Wave 10 expansion)
      │
      ▼
...
Wave 8 established scenario-enforced operator depth for eight profiles. Wave 10 expands the current registry to twelve with `prompt-injection-boundary-analysis` as the ninth profile, `rag-memory-data-isolation-analysis` as the tenth, `connector-plugin-trust-analysis` as the eleventh, and `tool-capability-and-confirmation-analysis` as the twelfth:
