...
This raises the current registry from eight Wave 8 profiles to **thirteen CI-enforced profiles** while keeping the Wave 8 history intact. Wave 10 profiles apply the same common operator-depth contract and add domain-specific causal reasoning rather than payload volume or checklist length.
